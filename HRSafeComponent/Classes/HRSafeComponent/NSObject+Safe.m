...
static  HRSafeCatchLogType     ls_safe_logType = HRSafeCatchLogTypeAll;
static  HRSafeCatchBlock       lsSafeProtectorBlock;
    ...
    if ([NSStringFromClass([self class]) isEqualToString:@"NSObject"]) {
        ...
        dispatch_once(&onceToken, ^{
            ...
            if (isDebug) {
                ...
                ls_safe_logType  = HRSafeCatchLogTypeNone;
            }
            lsSafeProtectorBlock=block;
        });
    }else{
        //只有NSObject 能调用openAllSafeProtector其他类调用没效果
        HRLog(@"------- 请用  [NSObject openAllSafeProtectorWithBlock:] 调用此方法");
    }
...
- (void)safe_forwardInvocation:(NSInvocation *)anInvocation{
    @try {
        ...
        HRCatchCrashLog(exception,HRSafeCatchCrashTypeSelector);
    } @finally {
    }
}
#pragma mark - 交换类方法
...
+ (void)safe_logCrashWithException:(NSException *)exception crashType:(HRSafeCatchCrashType)crashType
{
    ...
    if (lsSafeProtectorBlock) {
        lsSafeProtectorBlock(newException,crashType,fullMessage);
    }
    HRSafeCatchLogType logType = ls_safe_logType;
    if (logType == HRSafeCatchLogTypeNone) {
        
    }
    else if (logType == HRSafeCatchLogTypeAll) {
        HRLog(@"%@", fullMessage);
