...
-(void)safe_observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    @try{
        ...
        HRCatchCrashLog(exception,HRSafeCatchCrashTypeKVO);
    }@finally{
...
+(void)safe_dealloc_crash:(NSString*)classAddress
{
    ...
    for (NSString *key in dic) {
        ...
        if (array.count>0) {
            ...
            NSException *exception=[NSException exceptionWithName:@"KVO crash" reason:reason userInfo:nil]; HRCatchCrashLog(exception,HRSafeCatchCrashTypeKVO);
            }
        }
    }
